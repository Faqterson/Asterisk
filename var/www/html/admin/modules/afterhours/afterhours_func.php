...
function afterhours() {
   ...
   $row=db_query("SELECT afterhours.description,didnumber,announcement,afterhours.destination,afterhours.uniqueid from afterhours LEFT JOIN inbound ON (inbound.uniqueid = didid) LIMIT $offset, $rowsperpage", array());
   ...
   foreach($row as $key => $value)
   {
      ...
      echo "<td align='left'>";
      echo "<a style='background: none' href='index.php?mod=modify_afterhours&id=" . $value['uniqueid'] . "' title=''><img src=img/edit-icon.png height='20' width='20'></a>&nbsp";
      echo "<a style='background: none' href='index.php?mod=delete_afterhours&id=" . $value['uniqueid'] . "' title=''><img class='delete' src=img/delete-icon.png height='20' width='20'></a>";
      echo "</td>";      
      echo "</tr>";
   }
   echo "</table>";
?>
   ...
   <tr>
      <td align="center">
      <?php
      $resultPage = db_query("SELECT afterhours.description,didnumber,announcement,afterhours.destination from afterhours LEFT JOIN inbound ON (inbound.uniqueid = didid)", array());
      pagination('index.php?mod=afterhours',$rowsperpage,$resultPage,$page);                
      ?>
      </td>
   </tr>
   </table>
<?php
   echo "<form action='index.php?mod=add_afterhours' method='post'>";
   echo "<input class='button' type='submit' value='Add Afterhours' />";
   echo "</form>";

}

function add_afterhours() {
?>
<form name="edit" id="edit" action="index.php?mod=add_afterhours_details" method="post" enctype="multipart/form-data" class="" >
   <input name="mod" id="mod" value="add_afterhours_details" type="hidden">
...
?>
   ...
   <tr>
      <td></td>
      <td colspan="4">
         <input class="button" type="submit" value="Add Afterhours"</input>
      </td>
   </tr>
...
function add_afterhours_details() {
   ...
   if (isset($_POST['recordrequest'])) {
      ...
      if ($row[0]['max'] != null) {
         db_query("INSERT INTO extensions (context,exten,priority,app,appdata) VALUES (?,?,?,?,?)", array('extensions','*'.$recordcode,'1','Gosub','record-prompt,s,1('.$ddi.'-afterhours)'));
         $_FILES["announcement"]["name"] = $ddi."-afterhours.wav";
         echo "<script>alert('Your recording code is *$recordcode');</script>";
      } else {
         db_query("INSERT INTO extensions (context,exten,priority,app,appdata) VALUES (?,?,?,?,?)", array('extensions','*1000','1','Gosub','record-prompt,s,1('.$ddi.'-afterhours)'));
         $_FILES["announcement"]["name"] = $ddi."-afterhours.wav";
   ...
   db_query("INSERT INTO afterhours (description,didid,announcement,destination,data,recordrequest) VALUES (?,?,?,?,?,?)", array($description,$didid,$announcement,$destination,$dataid,$recordrequest));

   afterhours();
}

function modify_afterhours() {

   $ID = $_GET['id'];
   $row = db_query("SELECT afterhours.description,didnumber,announcement,afterhours.destination,afterhours.data,recordrequest from afterhours LEFT JOIN inbound ON (inbound.uniqueid = didid) WHERE afterhours.uniqueid = ?", array($ID));

?>
<form name="edit" id="edit" action="index.php?mod=modify_afterhours_details" method="post" enctype="multipart/form-data" class="" >
...
?>
   ...
   <tr>
      <td></td>
      <td colspan="4">
         <input class="button" type="submit" value="Modify Afterhours"</input>
      </td>
   </tr>
...
function modify_afterhours_details() {
   ...
   if (isset($_POST['recordrequest'])) {
      ...
      $row = db_query("SELECT exten FROM extensions WHERE appdata like ?", array('%'.$ddi.'-afterhours%'));
      $exten = $row[0]['exten'];
      if ($row[0]['exten'] != null) {
         $_FILES["announcement"]["name"] = $ddi."-afterhours.wav";
   ...
   db_query("UPDATE afterhours SET description = ?,didid = ?,announcement = ?,destination = ?,data = ?,recordrequest = ? WHERE uniqueid = ?", array($description,$didid,$announcement,$destination,$dataid,$recordrequest,$id));

   afterhours();
}

function delete_afterhours() {

   $id = $_GET['id'];

   $row = db_query("SELECT didnumber from inbound LEFT JOIN afterhours ON (inbound.uniqueid = didid) WHERE afterhours.uniqueid = ?", array($id));
   $didnumber = $row[0]['didnumber'];

   db_query("DELETE FROM extensions WHERE appdata like ?", array('%'.$didnumber.'-afterhours%'));

   db_query("DELETE FROM afterhours WHERE uniqueid = ?", array($id));
