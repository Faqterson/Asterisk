...
function officehours() {
   ...
   $row=db_query("SELECT officehours.description,didnumber,announcement,officehours.destination,officehours.uniqueid from officehours LEFT JOIN inbound ON (inbound.uniqueid = didid) LIMIT $offset, $rowsperpage", array());
   ...
   foreach($row as $key => $value)
   {
      ...
      echo "<td align='left'>";
      echo "<a style='background: none' href='index.php?mod=modify_officehours&id=" . $value['uniqueid'] . "' title=''><img src=img/edit-icon.png height='20' width='20'></a>&nbsp";
      echo "<a style='background: none' href='index.php?mod=delete_officehours&id=" . $value['uniqueid'] . "' title=''><img class='delete' src=img/delete-icon.png height='20' width='20'></a>";
      echo "</td>";
      echo "</tr>";
   }
   echo "</table>";
?>
   ...
   <tr>
      <td align="center">
      <?php
      $resultPage = db_query("SELECT officehours.description,didnumber,announcement,officehours.destination from officehours LEFT JOIN inbound ON (inbound.uniqueid = didid)", array());
      pagination('index.php?mod=officehours',$rowsperpage,$resultPage,$page);                
      ?>
      </td>
   </tr>
   </table>
<?php
   echo "<form action='index.php?mod=add_officehours' method='post'>";
   echo "<input class='button' type='submit' value='Add Officehours' />";
   echo "</form>";

}

function add_officehours() {
?>
<form name="edit" id="edit" action="index.php?mod=add_officehours_details" method="post" enctype="multipart/form-data" class="" >
...
?>
   ...
   <tr>
      <td></td>
      <td colspan="4">
         <input class="button" type="submit" value="Add Officehours"</input>
      </td>
   </tr>
...
function add_officehours_details() {
   ...
   if (isset($_POST['recordrequest'])) {
      ...
      if ($row[0]['max'] != null) {
         db_query("INSERT INTO extensions (context,exten,priority,app,appdata) VALUES (?,?,?,?,?)", array('extensions','*'.$recordcode,'1','Gosub','record-prompt,s,1('.$ddi.'-officehours)'));
         $_FILES["announcement"]["name"] = $ddi."-officehours.wav";
         echo "<script>alert('Your recording code is *$recordcode');</script>";
      } else {
         db_query("INSERT INTO extensions (context,exten,priority,app,appdata) VALUES (?,?,?,?,?)", array('extensions','*1000','1','Gosub','record-prompt,s,1('.$ddi.'-officehours)'));
         $_FILES["announcement"]["name"] = $ddi."-officehours.wav";
   ...
   db_query("INSERT INTO officehours (description,didid,announcement,destination,data,recordrequest) VALUES (?,?,?,?,?,?)", array($description,$didid,$announcement,$destination,$dataid,$recordrequest));
   
   officehours();
}

function modify_officehours() {

   $ID = $_GET['id'];
   $row = db_query("SELECT officehours.description,didnumber,announcement,officehours.destination,officehours.data,recordrequest from officehours LEFT JOIN inbound ON (inbound.uniqueid = didid) WHERE officehours.uniqueid = ?", array($ID));

?>
<form name="edit" id="edit" action="index.php?mod=modify_officehours_details" method="post" enctype="multipart/form-data" class="" >
...
?>
   ...
   <tr>
      <td></td>
      <td colspan="4">
         <input class="button" type="submit" value="Modify Officehours"</input>
      </td>
   </tr>
...
function modify_officehours_details() {
   ...
   if (isset($_POST['recordrequest'])) {
      ...
      $row = db_query("SELECT exten FROM extensions WHERE appdata like ?", array('%'.$ddi.'-officehours%'));
      $exten = $row[0]['exten'];
      if ($row[0]['exten'] != null) {
         $_FILES["announcement"]["name"] = $ddi."-officehours.wav";
   ...
   db_query("UPDATE officehours SET description = ?,didid = ?,announcement = ?,destination = ?,data = ?,recordrequest = ? WHERE uniqueid = ?", array($description,$didid,$announcement,$destination,$dataid,$recordrequest,$id));

   officehours();
}

function delete_officehours() {

   $id = $_GET['id'];

   $row = db_query("SELECT didnumber from inbound LEFT JOIN officehours ON (inbound.uniqueid = didid) WHERE officehours.uniqueid = ?", array($id));
   $didnumber = $row[0]['didnumber'];

   db_query("DELETE FROM extensions WHERE appdata like ?", array('%'.$didnumber.'-officehours%'));

   db_query("DELETE FROM officehours WHERE uniqueid = ?", array($id));
